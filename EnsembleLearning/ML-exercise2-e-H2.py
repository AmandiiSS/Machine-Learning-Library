...
def One_random_decisiontree(S,Attributes,Label,gain,depth,FeatureSetSize):
    ...
    OneMoreTrees = randomforest.RandomForest(Sb, Attributes, Label, gain, depth,1,FeatureSetSize)
    return OneMoreTrees
...
for i in range(100):
    ...
    Trees_randomforest = randomforest.RandomForest(S_rand,Attributes,Label,gain,depth,500,4)
    RandomForest_Predictors.append(Trees_randomforest)
    Single_Trees.append(Trees_randomforest[0])
...
with open(CSVfile, 'r') as f:
    for line in f:
        ...
        #RANDOM TREES
        average_pred_rt_i = 0
        for rt in RandomForest_Predictors:
            predicted_label_rand_tree = randomforest.prediction_RandomForest(rt, test) 
            if predicted_label_rand_tree == "no":
                average_pred_rt_i -= 1
            else:
                average_pred_rt_i += 1
        average_pred_rt_i = average_pred_rt_i/100
        bias_rt_i = pow(average_pred_rt_i-real_label,2)
        var_sum_rt_i = 0
        for rt in RandomForest_Predictors:
            predicted_label_rand_tree = randomforest.prediction_RandomForest(rt, test) 
            if predicted_label_rand_tree == "no":
                var_sum_rt_i += pow((-1-average_pred_rt_i),2)
            else:
                var_sum_rt_i += pow((1-average_pred_rt_i),2)
        samp_var_rt_i = (1/99)*var_sum_rt_i
        bias_rt_all.append(bias_rt_i)
        samp_var_rt_all.append(samp_var_rt_i)
...
#The final bias and variance for the random trees
bias_rt_final = sum(bias_rt_all)/len(bias_rt_all)
sample_var_rt_final = sum(samp_var_rt_all)/len(samp_var_rt_all)
expected_error_wrt_rt = bias_rt_final + sample_var_rt_final
...
print("Random trees results:")
print("Bias: ", bias_rt_final)
print("Variance: ", sample_var_rt_final)
print("Expected error wrt: ", expected_error_wrt_rt)
