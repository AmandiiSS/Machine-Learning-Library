...
Trees_bagged = bagged.Bagged_decisiontree(S,Attributes,Label,gain,max_depth,50)
print("Tree built!")
CSVfile = 'data/test.csv'
with open(CSVfile, 'r') as f:
    for line in f:
        ...
        predicted_label = bagged.prediction_Bagging(Trees_bagged,test)
        data.append({"ID":identifier,"Prediction":predicted_label})
        #print(data)
        
with open('predictions/submission_Amanda_SS_bagged_T50_fulltree_gini_differentintervals_quantiles_?common-attrib-same-lab.csv', 'w', newline='') as file:
