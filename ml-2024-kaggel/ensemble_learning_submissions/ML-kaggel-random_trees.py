...
Trees_bagged = randomforest.RandomForest(S,Attributes,Label,gain,max_depth,50,4)
print("Tree built!")
CSVfile = 'data/test.csv'
with open(CSVfile, 'r') as f:
    for line in f:
        ...
        predicted_label = randomforest.prediction_RandomForest(Trees_bagged,test)
        data.append({"ID":identifier,"Prediction":predicted_label})
        #print(data)
        
with open('predictions/submission_Amanda_SS_random_T50_fulltree_gini_differentintervals_quantiles_?common-attrib-same-lab.csv', 'w', newline='') as file:
